...
export class CommentsService {
    ...
    async create(exhibitId: number, text: string, user: User) {
        const exhibit = await this.exhibitsService.incrementCommentsCount(exhibitId);
    ...
    private async findCommentWithExhibit(commentId: number, exhibitId: number) {
        ...
        if (comment.exhibit.id !== exhibitId) {
            throw new HttpException(`Comment with ID ${commentId} is not associated with exhibit ${exhibitId}`, HttpStatus.BAD_REQUEST);
        }
